...
const format = (date, fm) => dayjs(date).format(fm);
const FlopTime = () => {
  const contentRef = useRef(null);
  const flipObjs = useRef([]);

  useEffect(() => {
    if (contentRef.current) {
      let flips = contentRef.current.children;
...
class Flipper {
  ...
  // 设置前牌文字
  _setFront(num, className) {
    this.allNumberClass.forEach((numberClass) => {
      if (
  ...
  _flip(type, frontNum, front, backNum, back) {
    // 如果处于翻转中，则不执行
    if (this.isFlipping) {
      return false;
    }
  ...
  // 下翻牌
  flipDown(frontNum, front, backNum, back) {
